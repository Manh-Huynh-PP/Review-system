...
import { normalizeDriveUrl, extractDriveFileId, isDriveVideoUrl, extractDriveVideoId, getDriveEmbedUrl } from '@/utils/googleDrive'
  ...
  return (
    <>
      ...
      {file?.type === 'sequence' && frameDetailView !== null && (() => {
        ...
        const frameUrl = u[frameDetailView]
        const isVideoFrame = frameUrl && isDriveVideoUrl(frameUrl)
        const resolvedFrameUrl = isVideoFrame
          ? getDriveEmbedUrl(extractDriveVideoId(frameUrl)!)
          : frameUrl
        const frameType = isVideoFrame ? 'video' : 'image'
